  ...
  module Adapters
    class RoleAdapter < RoleAdapterBase
      ...
      def where(relation, *args)
        conditions, values = build_query_scope(relation, args)
      ...
      def build_query_scope(relation, args)
        conditions = []
        values = []
        args.each do |arg|
          if arg.is_a?(Hash)
            c, b = build_query(arg[:name], arg[:resource])
          elsif arg.is_a?(String) || arg.is_a?(Symbol)
            c, b = build_query(arg.to_s)
          else
      ...
      def build_query(role, resource = nil)
        role_table = mappings[:role][:plural_relation_name]
        user_role_table = mappings[:user_role][:plural_relation_name]

        return [ "#{role_table}.name = ?", [role] ] if resource == :any

        query = "((#{role_table}.name = ?) AND (#{user_role_table}.resource_type IS NULL) AND (#{user_role_table}.resource_id IS NULL))"
        values = [ role ]

        if resource
          query.insert(0, '(')
          query += " OR ((#{role_table}.name = ?) AND (#{user_role_table}.resource_type = ?) AND (#{user_role_table}.resource_id IS NULL))"
          values << role << ( resource.is_a?(Class) ? resource.to_s : resource.class.name )

          if !resource.is_a?(Class)
            query += " OR ((#{role_table}.name = ?) AND (#{user_role_table}.resource_type = ?) AND (#{user_role_table}.resource_id = ?))"
